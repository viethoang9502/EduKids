...
export class GameSoccerComponent implements OnInit, AfterViewInit  {
  ...
  lessonId: number = 0;
  lessonGames: any[] = []; // Khai báo mảng lessonGames để chứa các trò chơi
  currentGameIndex: number = 0; // Index của trò chơi hiện tại
  score: number = 0; // Add score tracking
  pointsPerQuestion: number = 0; // Điểm mỗi câu hỏi
  ...
  ngOnInit(): void {
    ...
    this.loadGame(this.currentGameIndex); // Tải trò chơi đầu tiên
  }
  ...
  loadGame(index: number): void {
    this.lessonService.getDetailProduct(this.lessonId).subscribe({
      next: (response) => {
        this.lessonGames = response.data.lesson_games; // Update the lessonGames array with all games
        if (response.data && response.data.lesson_games.length > 0) {
          this.pointsPerQuestion = 100 / response.data.lesson_games.length; // Tính điểm mỗi câu hỏi
          const gameData = response.data.lesson_games[index].game_data;
          try {
            const parsedData = JSON.parse(gameData);
            this.correctOption = parsedData.correctOption;
            this.options = this.shuffleArray(parsedData.options); // Cập nhật danh sách options từ API và ngẫu nhiên hóa
...
shoot(optionIndex: number): void {
  ...
  if (optionIndex === this.correctOption) {
      ...
      this.animateBall(optionIndex, false);
      this.score += this.pointsPerQuestion; // Increment score for correct answer
      // Thủ môn không bay, bóng bay vào
  } else {
...
animateBall(optionIndex: number, goalkeeperShouldMove: boolean): void {
 ...
 nextGame(): void {
    if (this.currentGameIndex < this.lessonGames.length - 1) {
      this.currentGameIndex++;
      this.loadGame(this.currentGameIndex);
    } else {
      alert('Đây là trò chơi cuối cùng.');
      console.log('Total Score:', this.score); // Hiển thị tổng điểm khi kết thúc tất cả các câu hỏi
      this.scoreService.setScore(this.score); // Set the score
    }
  }
}